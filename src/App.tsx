    ...
    <div style={{ minHeight:"100vh", background:"var(--bg)", fontFamily:"'Barlow','Barlow Condensed',sans-serif", color:TTEXT }}>
      ...
      <header style={{ background:darkMode?"linear-gradient(135deg,#060d1a,#0a1628)":"linear-gradient(135deg,#1a3a5c,#1e4976)", borderBottom:`3px solid ${ORANGE}`, padding:"0 12px", height:68, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, boxShadow:"0 4px 24px #00000088", gap:8 }}>
        ...
        <div style={{ display:"flex", gap:5, alignItems:"center", flexShrink:0 }}>
          ...
          {view === "dashboard" && <>
            <button className="hide-mobile" onClick={()=>{ playClick("soft"); loadData(); }} style={{ ...btn, background:"transparent", color:"var(--sub)", border:`1px solid var(--border)`, borderRadius:7, padding:"7px 9px", fontSize:14 }} title="Aggiorna">🔄</button>
      ...
      <main className="main-pad" style={{ maxWidth:740, margin:"0 auto", padding:"20px 14px" }}>
        ...
        {view === "dashboard" && (
          <div className="anim">
            ...
            <div style={{ display:"flex", marginBottom:18, border:`1px solid var(--border)`, borderRadius:10, overflow:"hidden" }}>
              {[{key:"active",label:"⚠ GUASTI",color:ORANGE,count:reports.length},{key:"fuoriuso",label:"🔧 FUORI USO",color:"var(--fuori-uso-text)",count:fuoriUso.length},{key:"resolved",label:"✅ RISOLTI",color:GREEN,count:resolved.length}].map(t=>(
