...
function MezzoSelect({ value, isCustom, onSelect, onCustomToggle, onCustomChange, error, accent }:
  { value: string; isCustom: boolean; onSelect: (label: string) => void; onCustomToggle: () => void; onCustomChange: (v: string) => void; error?: boolean; accent: string }) {
  return (
    <>
      {!isCustom ? (
        <select value={value} onChange={e=>{ e.target.value===MEZZI_CUSTOM ? onCustomToggle() : onSelect(e.target.value); }}
          style={{ width:"100%", background:"var(--input-bg)", color:value?"#cce0f5":"#2a4a6e", border:`1px solid ${error?RED:BORDER}`, borderRadius:8, padding:"12px 14px", fontSize:14, fontFamily:"inherit" }}>
          <option value="">— Seleziona mezzo —</option>
          {VEHICLE_TYPES.map(cat => (
            <optgroup key={cat} label={cat}>
              {MEZZI.filter(m=>m.categoria===cat).map(m => (
                <option key={mezzoLabel(m)} value={mezzoLabel(m)}>{m.descrizione} — n°{m.numero}{m.targa ? " — "+m.targa : ""}</option>
              ))}
            </optgroup>
          ))}
          <option value={MEZZI_CUSTOM}>✎ Mezzo non in elenco (inserisci manualmente)</option>
        </select>
      ) : (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                ...
                <div style={{ flex:1, minWidth:140 }}>
                  ...
                  <MezzoSelect value={form.plate} isCustom={plateCustom} error={errors.plate} accent={ORANGE}
                    onSelect={v=>{
                  ...
                  <div style={{ background:"var(--fuori-uso-bg)", border:"1px solid var(--fuori-uso-text)33", borderRadius:10, padding:"16px", display:"flex", flexDirection:"column", gap:12 }}>
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                      <div style={{ flex:1, minWidth:130 }}>
                        <Label text="Targa / ID Mezzo *"/>
                        <MezzoSelect value={fuForm.plate} isCustom={fuPlateCustom} error={fuErrors.plate} accent={YELLOW}
                          onSelect={v=>{
                            const m = MEZZI_BY_LABEL.get(v);
